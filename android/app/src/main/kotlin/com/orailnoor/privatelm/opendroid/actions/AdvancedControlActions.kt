...
import com.orailnoor.privatelm.opendroid.accessibility.GenericAppAutomator
    ...
    private class CopyFileAction : Action {
        ...
        private fun copyRecursively(src: File, dest: File) {
            if (src.isDirectory) {
                ...
                src.listFiles()?.forEach { file -> copyRecursively(file, File(dest, file.name)) }
            } else {
    ...
    private class ZipFilesAction : Action {
        ...
        private fun zipRecursively(root: File, file: File, zos: java.util.zip.ZipOutputStream) {
            ...
            if (file.isDirectory) {
                ...
                zos.closeEntry()
                file.listFiles()?.forEach { child -> zipRecursively(root, child, zos) }
            } else {
